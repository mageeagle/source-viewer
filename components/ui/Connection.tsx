...
function Connection() {
  ...
  const { sourceNo, setSourceNo } = useUser(
    useShallow((state) => ({
      sourceNo: state.sourceNo,
      setSourceNo: state.setSourceNo,
    }))
  );
  const { speakerNo, setSpeakerNo } = useUser(
    useShallow((state) => ({
      speakerNo: state.speakerNo,
      setSpeakerNo: state.setSpeakerNo,
    }))
  );
      ...
      <div className={innerWrapper + " max-h-screen"}>
        ...
        <div className="my-2">
          ...
          <input
            ...
            type="number"
            value={sourceNo}
            onChange={(e) => setSourceNo(Number(e.target.value))}
          />{" "}
        </div>
        <div className="my-2">
          {" "}
          ...
          <input
            ...
            type="number"
            value={speakerNo}
            onChange={(e) => setSpeakerNo(Number(e.target.value))}
          />{" "}
        </div>
