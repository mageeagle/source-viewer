...
export default function SpeakerSource ({ index }: {index: number}) {
  ...
  useEffect(() => {
    ...
    useUser.getState().setNestedZus('speakerColor', index, {r: 125, g: 125, b: 125, a:1})
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  useEffect(() => {
    if (!osc) return
    const xyz = osc.on('/speaker/' + (index) + '/xyz', (message: { args: any }) => {
      vec.current.fromArray([message.args[0], message.args[2], -message.args[1]])
    })
    const color = osc.on('/speaker/' + (index) + '/color', (message: { args: any }) => {
      useUser.getState().setNestedZus('speakerColor', index, { r: message.args[0] * 255, g: message.args[1] * 255, b: message.args[2] * 255, a: message.args[3] })
    })
    return () => {
