...
export const subKey = (hook: UseBoundStore<any>, key1: MutableRefObject<any>, key2: string | number) => {
  hook.subscribe((state: { [x: string | number]: any; }) => (key1.current = state[key2]))
}

export const subNestedKey = (hook: UseBoundStore<any>, key1: MutableRefObject<any>, key2: string | number, key3: string | number) => {
  hook.subscribe((state: { [x: string | number]: { [x: string | number]: any; }; }) => (key1.current = state[key2][key3]))
}

// Set type for store / Only for typescript
interface StoreState {
  bgColor: string;
  sourceColor: {[index: number]: {r: number, g: number, b: number, a: number}};
  speakerColor: {[index: number]: {r: number, g: number, b: number, a: number}};
  osc: OSC | null,
  initialized: boolean,
  started: boolean,
  about: boolean,
  ...
  setSourceColor: (color: {r: number, g: number, b: number, a: number}) => void,
  setSpeakerColor: (color: {r: number, g: number, b: number, a: number}) => void,
...
export const useUser = create<StoreState>()(immer((set) => ({
  ...user,
  ...
  setNestedZus: (key1, key2, hi) => set(produce((state) => {
    state[key1][key2] = hi
  })),
